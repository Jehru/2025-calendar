...
// Calculate the current day index in the year
...
for (let i = 0; i < totalDays; i++) {
  ...
  if (i <= currentDayIndex) {
    dot.style.backgroundColor = "white"; // Fill past days and today
  }
  if (thisDay.toDateString() === currentDate.toDateString()) {
    dot.classList.add("current"); // Mark the current day
  }
  ...
  dot.addEventListener("mouseleave", () => {
    const allDots = document.querySelectorAll(".dot");
    allDots.forEach((d, index) => {
      if (index > currentDayIndex && !d.classList.contains("current")) {
        d.style.backgroundColor = "#333";
      }
    });

    // Reset days left to default
    const defaultDaysLeft = totalDays - currentDayIndex - 1;
...
const initialDaysLeft = totalDays - currentDayIndex - 1;
